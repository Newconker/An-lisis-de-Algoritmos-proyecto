...
function resolverBotonEstrella() {
    if (estadoGame == true) {
        ...
        modoAestrella = true
        aStar(matrizCentral,1);
  
        aStar(matrizInferiorD,2);
        aStar(matrizInferiorI,3);
        aStar(matrizSuperiorD,4);
        aStar(matrizSuperiorI,5);
    ...
    if (empezoReco == true) { //no completa sodoku, solo avanza en el tablero 
       ...
       if (evaluandoC != largoCentral+1) {
        ...
        if ( banderaSI == true && bii == 0){
             if (evaluandoII != LargoInferiorIzquierda) {
                evaluandoII ++
...
function aStar(matriz,num) {
    console.log("Se esta haciendo a estrella")
    let openList = [];
    let closeList = [];
    let actual = matriz;
